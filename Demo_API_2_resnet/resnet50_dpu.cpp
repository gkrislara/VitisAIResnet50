...
int dpuSetInputImageWithScale(DPUTask *task, const char* nodeName, const cv::Mat &image, float *mean, float scale=1.0f, int idx=0)
{
    int value;
    int8_t *inputAddr;
    ...
    channel = dpuGetInputTensorChannel(GetInputTensorChannel(task, nodeName, idx);
...
void runResnet50(string imgpath, DPUTask *taskConv) {
  ...
  int channel = dpuGetOutputTensorChannel(taskConv, OUTPUT_NODE);
  float scale = dpuGetInputTensorScale(taskConv,INPUT_NODE);
  float *softmax = new float[channel];
  float *FCResult = new float[channel];
  float meanRGB[3]={123.0f,117.0f,104.0f};                         
  ...
  for (auto &imgpath : images) {
    ...
    cout << "Input Image size: " << img.cols << " x " << img.rows << " x " << img.channels()
         << endl;
    cout << "Input featuremap: " << dpuGetInputTensorWidth(taskConv, INPUT_NODE) << " x "
         << dpuGetInputTensorHeight(taskConv,INPUT_NODE) << endl;
    _T(dpuSetInputImageWithScale(taskConv, INPUT_NODE, img, meanRGB));

    _T(dpuRunTask(taskConv));

    /* Get FC result and convert from INT8 to FP32 format */
    _T(dpuGetOutputTensorInHWCFP32(taskConv, OUTPUT_NODE, FCResult, channel));
...
int main(int argc, char **argv) {
  if (argc != 2) {
    cout << "Usage of resnet50 demo: ./resnet50_tf file_name" << endl;
